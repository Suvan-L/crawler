...
CREATE TABLE `crawler_company_list` (
    ...
    `reg_no` varchar(80) NULL COMMENT '[选填] 注册号（可能为 null）',
    `le_rep` varchar(10) NOT NULL COMMENT '[必填] 法定代表人',
    `est_date` timestamp NOT NULL COMMENT '[必填] 成立日期',
    `op_state_ccn` varchar(50) NOT NULL COMMENT '[必填] 企业状态',
    `reg_capital` decimal(16, 2) NOT NULL COMMENT '[必填] 注册资金（2 位小数点）（万元为单位）',
    `re_capital_coin` varchar(10) NOT NULL COMMENT '[必填] 注册资金单位(例：人名币元)',
