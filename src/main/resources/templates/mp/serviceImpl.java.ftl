...
public class ${table.serviceImplName} extends ${superServiceImplClass}<${table.mapperName}, ${entity}> implements ${table.serviceName} {
    ...
    @Override
    public Page<${entity}> listsByPage(int page, int pageSize, String factor) {
    ...
    public ${entity} getById(int id) {
        log.info("正在查询 ${entity?uncap_first} 中 id : {} 的数据" , id);
        ${entity} ${entity?uncap_first} = super.getById(id);
        log.info("查询 id : {} 的 ${entity?uncap_first} {}",id,(null == ${entity?uncap_first}?"无结果":"成功"));
    ...
    public int deleteById(int id) {
        log.info("正在删除 id:{} 的 ${entity?uncap_first}" , id);
        if (super.removeById(id)) {
            log.info("删除 id:{} 的 ${entity?uncap_first}成功", id);
            return id;
        } else {
            log.error("删除 id:{} 的 ${entity?uncap_first}失败", id);
    ...
    public int update(${entity} ${entity?uncap_first}) {
        log.info("正在更新 id:{} 的 ${entity?uncap_first}", ${entity?uncap_first}.getId());
        if (super.updateById(${entity?uncap_first})) {
            log.info("更新d为{}的${entity?uncap_first}成功",${entity?uncap_first}.getId());
            return ${entity?uncap_first}.getId();
        } else {
            log.error("更新 id:{} 的 ${entity?uncap_first} 失败", ${entity?uncap_first}.getId());
